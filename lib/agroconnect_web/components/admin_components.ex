defmodule AgroconnectWeb.AdminComponents do
  use Phoenix.Component
  import Phoenix.HTML, only: [raw: 1]

  def admin_sidebar(assigns) do
    assigns = assign_new(assigns, :current_path, fn -> "/admin" end)

    ~H"""
    <aside class="flex flex-col h-screen bg-white shadow-lg border-r border-gray-200 w-64 fixed top-0 left-0 z-30">
      <!-- Top section with profile picture and admin badge -->
      <div class="flex flex-col items-center py-8 border-b border-gray-200">
        <div class="relative">
          ...
          <div class="absolute -bottom-1 -right-1">
            <span class="flex items-center justify-center h-6 w-16 rounded-full text-xs font-bold text-white bg-emerald-600 border-2 border-white">
              ADMIN
            </span>
          </div>
        </div>
    ...
    <!-- Main navigation -->
      <nav class="flex-1 py-6 px-4 space-y-1 overflow-y-auto">
        ...
        <.menu_item current_path={@current_path} path="/admin" title="Dashboard" />
        <.menu_item current_path={@current_path} path="/admin/profile" title="Profiles" />

        <div class="py-2 px-4">
          <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Management</h3>
        </div>
        <.menu_item current_path={@current_path} path="/admin/users" title="Users" />
        <.menu_item current_path={@current_path} path="/admin/vets" title="Vets" />
        <.menu_item current_path={@current_path} path="/admin/agrovets" title="Agrovets" />
        <.menu_item current_path={@current_path} path="/admin/roles" title="Roles" />

        <div class="py-2 px-4">
          <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider">System</h3>
        </div>
        <.menu_item current_path={@current_path} path="/admin/logs" title="Logs" />
      </nav>
    </aside>
    """
  end
  ...
  defp get_icon("/admin/profiles") do
    """
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
  ...
  defp get_icon("/admin/agrovets") do
    """
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
